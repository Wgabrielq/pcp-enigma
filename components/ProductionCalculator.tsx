...
import { Printer, Layers, Droplets, Scissors, AlertTriangle, CheckCircle, ArrowRight, ArrowUpRight, FileCheck, Lightbulb, AlertOctagon, CheckSquare, Search, Gauge, Edit2, X } from 'lucide-react';
...
const ProductionCalculator: React.FC = () => {
  ...
  const LayerSelector = ({ layerKey, spec, theoreticalKg, analysis }: { layerKey: string, spec: LayerSpec, theoreticalKg: number, analysis: any }) => {
        ...
        <div className="mb-6 bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
             <div className="flex justify-between items-start mb-3">
                 ...
                 <div className="text-right">
                     <span className={`text-xl font-bold ${isHeavier ? 'text-amber-600' : 'text-slate-800'}`}>{displayKg.toLocaleString()} Kg</span>
                 ...
                 {recommendations.map((rec, idx) => (
                     ...
                     >
                        <div>
                            ...
                            <div className="text-xs text-slate-600 flex space-x-2">
                                <span>{rec.material.width}mm</span>
                                <span>{rec.material.thickness}μ</span>
                                <span className={rec.material.currentStockKg < displayKg ? "text-red-500 font-bold" : "text-emerald-600 font-bold"}>
                        ...
                        {selectedMatId === rec.material.id && <CheckCircle size={18} className="text-brand-600"/>}
                     </div>
                 ))}
             </div>
