...
import { Client, Material, MaterialType, ProductRecipe, SystemConfig, ProductionOrder, OrderStatus, Supplier, LayerSpec } from '../types';
...
const load = <T>(key: string, defaults: T): T => {
  try {
    ...
    if (key === 'flexo_config') {
        ...
        // Deep merge for zetaConfig to ensure new SOAP fields exist if they were missing
        const mergedZeta = { ...(defaultsAny.zetaConfig || {}), ...(savedConfig.zetaConfig || {}) };
        return { ...defaultsAny, ...savedConfig, zetaConfig: mergedZeta } as T;
    }
